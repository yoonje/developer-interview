...
* 문맥교환시에 진행 사항을 PCB에 저장하고 CPU 반환 -> CPU를 할당받으면 PCB에 저장되어 있는 내용을 불러와 이전에 종료되었던 시점부터 다시 수행
...
* 동일한 자원에 동시에 접근하느 경우가 발생하는 코드 영역
...
## 블락킹과 논블락킹의 차이
### 👉 블락킹
* 주로 I/0 처리 함수가 끝날 때까지 `기다리고` 처리가 끝나면 결과를 리턴하는 방식
* 블락킹은 요청한 작업을 마칠 때까지 계속 기다림
