# Coding Interview
코딩 인터뷰 질문에 관련된 팁과 최소 개념입니다. 모든 내용을 자세하게 담기엔 가성비가 떨어지므로 모르는 부분이 나오신다면 검색을 통해 추가적인 학습을 추천합니다. 기본적인 CS 지식과 IT회사(N사,K사,L사), 통신회사(S사)의 인터뷰 경험을 참고하여 만들었습니다.
...
* TCP 3-way handshake가 언제 일어나고 어떤 과정인지
  ...
  * TCP는 양방향성 연결이기 때문에 발생하는 매커니즘으로 연결을 성립할 때는 서버가 준비가 다 된 상태인 대기 상태에서 시작하여 3-way로 해야 가능
* TCP 4-way handshake가 언제 일어나고 어떤 과정인지
  ...
  * Client -> Server : `ACK 전송`
  * TCP는 양방향성 연결이기 때문에 발생하는 매커니즘으로 연결 해제할 때는 서버가 준비가 되지 않은 상태이기 때문에 연결 해제 대기 상태로 만들기 위해서 4-way로 해야 가능
...
* RESTful API
  * `REST`는 `HTTP기반으로 필요한 자원에 접근하는 방식을 정해놓은 아키텍쳐`
